...
contract ErContract {
    ...
    mapping(string => Documents) drawings;
    ...
    function timelockcontract() internal {
        ...
        ErTokenTimelock timelock = new ErTokenTimelock(address(this));
    ...
    function addDocument(string memory _name, string memory _additionaldocsurl)
        public
    {
        drawings[_name] = Documents(_additionaldocsurl, now, msg.sender);
    }

    function getDocument(string memory _name)
        public
        view
        returns (string memory, uint256)
    {
        return (drawings[_name].documenturi, drawings[_name].timestamp);
    }
    ...
    function developmentRounds(
        ...
        ErTokenTimelock timelock = ErTokenTimelock(timelockcontractaddress);
...
contract ErTokenTimelock {
    ...
    {
        require(vestingstages[_name].starttime <= now);
